...
def submit_vqa_to_deepseek(llm, qa):
    prompt = f"You are a medical expert. Please answer the following multiple-choice question by selecting the most appropriate option.: \n {qa}.\n \
              You must return only the option identifier (e.g., '(A)') without any additional text, do not add any additional analysis, just return the correct option identifier. Please answer my question."

    # Define sampling parameters
    sampling_params = SamplingParams(
        temperature=0.1,   # Use temperature=0 for deterministic answers
        max_tokens=1024)
